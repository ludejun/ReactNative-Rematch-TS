...
class Monitor {
  ...
  init({ appName, appVersion, headerName, apiUrl, maxStorage, custno, maccode }) {
    // 持久存储Key、日志发送的Header Key
    this.storageName = headerName || '__ReactMonitorLogs'
    // 日志最大存储量
    this.maxStorage = maxStorage || 5
    // 存储事件日志
    this.ev = []
    // 日志发送地址，Get请求，一般为请求1*1gif
    this.apiServer = apiUrl || ''
    this.custno = custno || ''
    this.appName = appName || ''
    this.appVersion = appVersion || ''
    if (!this.apiServer) console.error('[Monitor报错]：埋点方法需要上报服务端URL，一般为请求1*1gif')
    try {
      // 如果缓存中有数据，初始化到this.ev
      const storage = window.localStorage.getItem(this.storageName)
      if (storage !== '[]' && storage !== null) this.ev = JSON.parse(storage)

      // 设置maccode时看缓存中有maccode
      if (maccode) this.maccode = maccode
      else {
        const macLocal = window.localStorage.getItem('maccode')
        if (!macLocal) this.maccode = macLocal
        else {
    ...
    this.baseInfo = this.baseInfo && Object.keys(this.baseInfo).length > 0
      ? this.baseInfo
  ...
  sendLog() {
    if (this.ev.length > 0) {
      ...
      if (this.ip && this.baseInfo.ip === undefined) this.baseInfo.ip = this.ip
      ...
      request.onreadystatechange = () => {
        ...
        } else console.error('[Monitor报错]：埋点上报不成功') // TODO 是否做超时重发
      }
      request.send(null)
    }
  }
  ...
  track({ id = '', type = 'PV', custom }) {
    const self = this
    if (type && id) return function decorator(...target) {
      console.log('target:', target)
      if (target.length === 1) return self.withTrackingComponentDecorator(
        type,
        id,
        custom,
      )(...target)
  ...
  generateNormal() {
    // 用户
    if (this.custno === '') this.custno = (JSON.parse(window.localStorage.getItem('userInfo')) || {}).custno
        || null
